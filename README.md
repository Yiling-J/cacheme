...
* **invalid_keys(or invalid_node)**
   ...
   * user tags change(will call invalidation on `user:{id}tags:invalid`)
   ...
   To make sure user won't see wrong results when book name changing or book tags changing or user updating
   their tags, we need 3 invalid_keys: `book:{id}:name:invalid`, `book:{id}:tags:invalid`, `user:{id}:tags:invalid`.
   `id` in `book:{id}:name:invalid` and `book:{id}:tags:invalid` are all book ids generated by server, so if
   result is `[{'id': 1, 'name': 'book1'}, {'id': 2, 'name': 'book2'}]`, and user id is 123, then all invalid_keys
